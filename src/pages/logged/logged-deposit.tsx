...
export function LoggedDeposit() {
  ...
  const largestKeys = useLargestBalanceKeys({
    coinSymbols: Object.keys(NetworkDictionary) as CoinSymbol[],
  }).keys.filter((k) => k.address !== 'No keys for this network');
  // TODO: remove once largestKeys are actually used for something
  console.log(largestKeys);
  return (
    <LoggedMain>
      <>
          ...
          <IonCol class="ion-center">
            <WrapperDiv>
              ...
              <IonItem class="address-item">
                <IonLabel>
                  <AddressTitle>{t('PublicAddress')}</AddressTitle>
                  <AddressContent>
                    XXXXXXXXXYYYYYYYYYY
